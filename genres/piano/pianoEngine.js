...
console.log("pianoEngine.js ver. 007 loaded");
...
export async function createPianoEngine(params) {
    ...
    // 1. SETUP GLOBALE (Sustain, Volume, BPM)
    Tone.Transport.stop();
    ...
    // Tocco umano basato sulla complessità
        ...
        for (let m = 0; m < section.measures; m++) {
            ...
            sectionProg.forEach((degree, i) => {
                ...
                for (let s = 0; s < stepsInChord; s++) {
                    ...
                    // --- MANO DESTRA (Accordi/Arpeggi) ---
                    if (style.rh[patternIdx] > 0) {
                        const h = (rand() - 0.5) * 0.03;
                        const dynamicVelBase = (p.velocityBase * style.rh[patternIdx]) + ((rand() - 0.5) * humanTouch);

                        if (style.type === "arpeggio") {
                            const note = chordNotes[s % chordNotes.length];
                            Tone.Transport.schedule((time) => {
                                piano.triggerAttackRelease(note, "4n", time, Math.max(0.1, Math.min(1, dynamicVelBase * 0.7)));
                            }, stepTime + h);
                        } else {
                            chordNotes.forEach((note, idx) => {
                                const strum = idx * 0.02;
                                const noteVel = dynamicVelBase + ((rand() - 0.5) * 0.1); // variazione per ogni dito
                                Tone.Transport.schedule((time) => {
