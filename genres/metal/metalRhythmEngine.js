// metalRhythmEngine.js — ver. 018 (Full Score Integration con Ballad support)
...
export function scheduleRhythm(section, progression, instruments, params, rand, measureDur, nextSectionRoot, score) {
    ...
    // Verifica se gli strumenti ballad sono disponibili (opzionali)
    ...
    const grooves = {
        intro: [
            "intro_ambient", "intro_heavy_strikes", "stratovarius_intro", "doom_slow",
            "cinematic_buildup", "industrial_static", "stoner_doom", "power_ballad",
            "ballad_intro_arpeggio", "intro_epic_pad", "intro_epic_timpani"
        ],
        ...
        outro: ["ballad_outro_open", "outro_sustain", "outro_timpani", "outro_final_hit", "outro_epic_hit", "outro_epic_roll", "outro_epic_finale"]
    };
    ...
    const currentGroove = getGroove(
        isIntro ? "intro" : isPreChorus ? "prechorus" : isChorus ? "chorus" : isOutro ? "outro" : "verse",
        energy, brightness, complexity
    );

    // Ballad groove solo se gli strumenti sono disponibili
    const isBalladGroove = hasBalladInstruments && currentGroove.startsWith("ballad_");
    ...
    for (let m = 0; m < section.measures; m++) {
        ...
        for (let s = 0; s < 16; s++) {
            ...
            if (isBalladGroove && acousticGuitar) {
                inst = acousticGuitar;
            } else if (isChorus) {
            ...
            switch (currentGroove) {
                ...
                case "intro_epic_pad":
                    if (s === 0) { playGuitar = true; inst = guitarOpen; sustain = true; kick = true; }
                    if (s === 0 && hasBalladInstruments && StStringPad) {
                ...
                case "outro_sustain":
                    if (s === 0) { playGuitar = true; inst = guitarOpen; sustain = true; kick = true; }
                    if (hasBalladInstruments && StStringPad) {
                        StStringPad.triggerAttackRelease(currentRoot + "3", "2n", absoluteTime);
                    }
                    break;
                case "outro_timpani":
                ...
                case "outro_epic_hit":
                    if (s === 0) { playGuitar = true; inst = guitarOpen; sustain = true; kick = true; snare = true; }
                    if (s === 0) { try { drums.player("crash2").start(absoluteTime); } catch(e){} }
                    break;
                ...
                // BALLAD GROOVES (solo se strumenti disponibili)
                case "ballad_intro_arpeggio":
                case "ballad_verse_arpeggio":
                    if (hasBalladInstruments) {
            ...
            // --- 2. SCHEDULAZIONE BATTERIA ---
            if (isBalladGroove && hasBalladInstruments) {
                // Batteria Ballad (minimal)
                Tone.Transport.schedule(time => {
                    if (s === 0) {
                        try { drums.player("kick").start(time); } catch(e) {}
                        if (score) score.addNote("Drums", "Kick", section.name);
                    }
                    if (s % 4 === 0) {
                        try { drums.player("hihat").start(time); } catch(e) {}
