...
import { normalizeNote, normalizePadNote } from "./metalInstruments.js";

console.log("metalRhythmEngine.js ver. 016.1 loaded");
...
if (isChorus) {
    ...
    if (s === 8) kick = true;
    // Pad
    const padNote = normalizePadNote(currentRoot + "3");
instruments.StStringPad.triggerAttackRelease(padNote, "2n", absoluteTime);
    break;
...
case "intro_timpani_roll":
    ...
    if (s === 0) {
        const padNote = normalizePadNote(currentRoot + "3");
instruments.StStringPad.triggerAttackRelease(padNote, "1n", absoluteTime);
    }
    break;
...
case "outro_epic_finale":
    ...
    default:
        ...
        break;
            ...
            // CORREZIONE per la sezione fill zone (RIGHE ~315-335)
...
if (isFillZone && complexity > 0.4) {
    ...
    if (isEpicOutro) {
        ...
        try { drums.player("tom" + (s - 11)).start(absoluteTime); } catch(e){}
    }
} 
