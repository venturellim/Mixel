...
console.log("padEngine.js ver. 2.3 loaded");
...
export function initPadEngine(instruments, metalParams, rand, imageParams) {
    ...
    function applyPadMicroMovement(section, profile) {
        ...
        Tone.Transport.scheduleRepeat(time => {
            ...
            ambientPad.set({ filter: { frequency: base + jitter } });
        }, "2n", start, end - start);
        ...
        Tone.Transport.scheduleRepeat(time => {
            ...
            breathingPad.set({ filter: { Q: depth } });
        }, "8n", start, end - start);

        // ChoirPad micro wet
        Tone.Transport.scheduleRepeat(time => {
            const jitter = 0.05 * Math.sin(time * 0.3);
            choirPad.set({ reverb: { wet: 0.7 + jitter } });
        }, "2n", start, end - start);
        ...
        Tone.Transport.scheduleRepeat(time => {
            ...
            counterPad.set({ filter: { frequency: base + jitter } });
        }, "8n", start, end - start);
    }
